/**
 * 站点子路径。经 Nginx /Music 反代时为 "/Music"，本机直连 8004 时为空。
 * @returns {string} 不带末尾斜杠的前缀
 */
function appBase() {
    const raw = (typeof window !== 'undefined' && window.APP_BASE) ? String(window.APP_BASE) : '';
    return raw.replace(/\/$/, '');
}

/**
 * 把应用内路径拼到子路径前缀上，例如 "/api/guilds" → "/Music/api/guilds"
 * @param {string} path - 以 / 开头的站内路径
 * @returns {string} 可供 fetch 使用的完整路径
 */
...
function loadChannels(guildId) {
    fetch(apiUrl(`/api/channels?guild_id=${guildId}`))
...
function loadPlaylist(guildId) {
    fetch(apiUrl(`/api/playlist/current?guild_id=${guildId}`))
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                updatePlaylist(data.playlist);
...
function searchMusic(keyword) {
    fetch(apiUrl(`/api/search?keyword=${encodeURIComponent(keyword)}`))
        .then(response => response.json())
        .then(data => {
            if (data.success) {
